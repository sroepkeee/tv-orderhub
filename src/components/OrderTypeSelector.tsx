  ...
  const loadOrderTypes = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('order_type_config').select('*').order('category', {
        ascending: true
      }).order('display_name', {
        ascending: true
      });
  ...
  return <div className="space-y-2">
      <Select value={value} onValueChange={onValueChange} disabled={disabled || loading}>
        ...
        <SelectContent className="max-h-[400px]">
          {Object.entries(groupedTypes).map(([category, types]) => <SelectGroup key={category}>
              <SelectLabel className="flex items-center gap-2 px-2 py-1.5">
                <Badge variant="outline" className={categoryColors[category]}>
                  {categoryLabels[category]}
                </Badge>
              </SelectLabel>
              {types.map(type => <SelectItem key={type.order_type} value={type.order_type}>
