...
export function OrderReturnForm({ 
  ...
  const handlePhotoUpload = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        const filePath = `returns/orders/${order.id}/${fileName}`;
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      // Create return request linked to order
      const { data: request, error: requestError } = await supabase
        .from('return_requests')
        .insert({
          organization_id: organizationId,
          order_id: order.id,
          requester_profile_id: userProfileId,
          customer_name: order.customer_name,
          customer_document: order.customer_document,
          destination_warehouse: formData.destinationWarehouse,
          destination_type: 'warehouse',
          total_weight_kg: parseFloat(formData.totalWeightKg) || null,
          total_volumes: parseInt(formData.totalVolumes) || 1,
          volume_details: formData.lengthCm && formData.widthCm && formData.heightCm ? [{
      ...
      // Insert items using order_item_id
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>Solicitar Retorno - NF {order.order_number}</DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-2">
              ...
              <Select 
                ...
                onValueChange={(v) => setFormData({ ...formData, destinationWarehouse: v })}
              >
              ...
              <div className="grid grid-cols-4 gap-2">
                ...
                <div>
                  <Select 
                    ...
                    onValueChange={(v) => setFormData({ ...formData, packagingType: v })}
                  >
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
