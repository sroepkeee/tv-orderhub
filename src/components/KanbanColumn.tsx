...
export const KanbanColumn = ({
  ...
  return <div ref={setNodeRef} className={`kanban-column flex-shrink-0 w-80 transition-all duration-300 flex flex-col ${isOver ? "drop-target" : ""}`}>
      {/* Column Header */}
      <div className={`${colorClass} rounded-t-lg p-4 sticky top-0 z-10 shadow-sm`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 bg-sky-200">
            <Icon className="h-5 w-5" />
            <h3 className="font-semibold text-base">{title}</h3>
          </div>
          <Badge variant="secondary" className="text-foreground bg-blue-600">
            {orders.length}
          </Badge>
        </div>
        {highCount > 0 && <div className="flex gap-2 mt-2">
