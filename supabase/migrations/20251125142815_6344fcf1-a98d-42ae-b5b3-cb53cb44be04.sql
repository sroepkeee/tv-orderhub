-- Criar nova função que considera order_category para determinar fase
...
-- Atualizar função get_phase_from_status para fallback correto
...
-- Atualizar política RLS de SELECT para usar a nova função
DROP POLICY IF EXISTS "Users can view orders in their phases" ON public.orders;
CREATE POLICY "Users can view orders in their phases" ON public.orders
FOR SELECT USING (
  has_role(auth.uid(), 'admin'::app_role) 
  OR (EXISTS (
    SELECT 1 FROM get_user_phases(auth.uid()) up
    WHERE up.phase_key = get_phase_from_order(orders.id)
  ))
  OR (auth.uid() = user_id)
);

-- Atualizar política RLS de UPDATE para usar a nova função
DROP POLICY IF EXISTS "Users can update orders in phases they can edit" ON public.orders;
CREATE POLICY "Users can update orders in phases they can edit" ON public.orders
FOR UPDATE USING (
  (EXISTS (
    SELECT 1 FROM get_user_phases(auth.uid()) up
    WHERE up.phase_key = get_phase_from_order(orders.id)
